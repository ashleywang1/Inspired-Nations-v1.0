...
/**
 * Calculates the value of x in 0 = c + bx + ax^2
 * @param a
 * @param b
 * @param c
 * @return
 */
public static BigDecimal quad(BigDecimal a, BigDecimal b, BigDecimal c, MathContext round) {
	BigDecimal twoa = new BigDecimal(2).multiply(a);
	return (b.negate()).subtract(BigDecimalUtils.sqrt((b.pow(2).subtract(
			new BigDecimal(4).multiply(a).multiply(c))))).divide(twoa,round);
